...
Race Condition: SQL Server takes longer to boot than the .NET application, causing the initial connection attempt to fail.

Networking Context: The terminal (running dotnet ef) runs on the "Host" VM, while the API runs inside a "Container." The Host needs localhost, but containers need the service name sqlserver.

Solution:

Resiliency: Implemented EnableRetryOnFailure() in Program.cs to handle the startup delay automatically.

Dual Connection Strings:

Used Server=sqlserver in docker-compose.yml for inter-container communication.
...
Added using Microsoft.EntityFrameworkCore.ChangeTracking; to the header of QuizDbContext.cs.

Implemented a ValueComparer<List<string>> in the OnModelCreating method to properly track element additions/removals.
...
Ran gh auth refresh -s codespace to update permissions.
...
docker compose down
🗄️ Entity Framework (EF) Core
Run these from the root of your repository or the specific service folder.
...
☁️ GitHub CLI (Remote Access)
Use these when switching between computers (Mac vs. Windows).
...
4. Setup Checklist for New Projects
