...
## Status
Foundation converged (v3); the context system is written, and the **UI trio is generated** from the Claude Design export (`design-system/`). Repo is **public** with CI + branch protection + auto-merge. **Layer-0 is complete** (framework pin, PostgreSQL migration, real scoring, identity/auth). The **frontend is built** — spec 0001 (auth + Manage Profile screens). The **production platform is built** — spec 0002 (single-origin host serving the SPA and `/api`, containerised local dev, expanded CI + CodeQL, CI-gated CD to Railway).

**Live at → https://quiztin-production.up.railway.app** (healthy).

> **⚠️ Deployment note (2026-07-24): the URL moved and CD is stale.** The app is live and healthy at the URL above, running the **last successful deploy (2026-07-20)**. The URL this README used to advertise (`gateway-production-02f8.up.railway.app`) was the **pre-0007 gateway service** and now 404s — 0007 folded the five services into one `Quiztin.Api` host (the `quiztin` service) but the old public URL was never updated here. Continuous deployment has also **failed on the ~3 merges to `main` since 07-20** (`railway up --service quiztin` fails in CI), so the live app is a few commits behind `main`. The repo-side deploy config (`railway.json`, `deploy.yml`, `src/Quiztin.Api/Dockerfile`) is correct; the CD break is Railway-side. Local dev (`docker compose up`) is unaffected. See `context/progress-log.md`.
