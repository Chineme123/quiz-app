...
> **⚠️ The live deployment is currently down (found 2026-07-24).** It deployed live through 2026-07-20, but the spec 0007 fold (five services → one `Quiztin.Api` host) was never reconciled on the Railway side: CD has failed on every merge since, and the old public URL (`gateway-production-02f8.up.railway.app`) now returns 404 "Application not found". The repo-side deploy config (`railway.json`, `deploy.yml`, `src/Quiztin.Api/Dockerfile`) is internally correct and consistent; the fix is Railway-dashboard work (stand up a working `quiztin` service, set its env, redeploy, then update this URL). Local dev (`docker compose up`) is unaffected. See `context/progress-log.md`.
