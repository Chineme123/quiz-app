...
> **Deployment note (2026-07-24).** The URL this README used to advertise (`gateway-production-02f8.up.railway.app`) was the **pre-0007 gateway service** and now 404s — 0007 folded the five services into one `Quiztin.Api` host (the `quiztin` service), and the live URL is the one above. CD was also **broken from 2026-07-20 to 07-24**: Central Package Management (adopted 07-20) needs `Directory.Packages.props` copied in the Dockerfile before `dotnet restore`, which it wasn't, so every deploy failed with `NU1015`. **Fixed in `0c3d403` and shipped in PR #84 — the deploy is green again and production is current.** See `context/progress-log.md` for the full incident.
