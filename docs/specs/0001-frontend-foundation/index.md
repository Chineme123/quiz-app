...
> **⚠️ Backend topology reconciled with [spec 0007](../0007-modular-monolith/index.md) (2026-07-18) and the corpus retirement (2026-07-24).** This spec was written against the five-microservice backend, so it names `AuthService` / `UserService` / `QuizService`, an `authdb` database, and the YARP gateway as the production origin. Spec 0007 folded all of that into **one `Quiztin.Api` host** with an **Identity** module (Auth + User merged, which fixed the profile FK gap by construction) and an **Assessment** module, over one `quiztin` Postgres database with a schema per module (`identity`, `quiz`). Everything that made this spec *a decision* still holds — the in-memory access token, the rotating `HttpOnly` refresh cookie, the tokens-only styling, and every acceptance criterion — only the topology names changed: where the text says a service or `authdb`, read the matching module and the `identity` schema, and where it says "the gateway", read the one host. The UC14 UI/UX brief this spec cites was retired with the corpus on 2026-07-24; the profile screen it specified is built. `verify.md` has been rewritten to run against the current single host.
...
1. ✅ **Done** (2026-07-10). AuthService refresh and logout: the `RefreshToken` entity, the `AddRefreshTokens` migration, the token service and repository, configuration for lifetimes and cookie flags, and the two controller endpoints. Move the hardcoded `AddHours(8)` in `JwtTokenService` to configuration. Verified with curl before any frontend exists. Satisfies **AC-1**, **AC-2**, **AC-3**, **AC-4**, **AC-5**. ⚠️ **AC-3's wording is unbuildable as written and needs `/architect` to correct it** (see `progress-log.md`, 2026-07-10). Code was in `src/Services/AuthService/`; since spec 0007 it lives in the Identity module (`src/Quiztin.Modules.Identity/`, tests in `tests/Quiztin.Modules.Identity.Tests/`).
2. ✅ **Done** (2026-07-10). Scaffold `frontend/`: Vite, TypeScript, the proxy route map, the Tailwind entry that binds to the tokens, ESLint with the accessibility plugin, Vitest, and Node entries in `.gitignore`. Satisfies **AC-6**. Verified: build, lint, tsc, one axe smoke test all green; dev server serves the app and the proxy forwards `/api/auth` to port 5005 (502 to the right origin, not a Vite 404); the compiled CSS shows `color:var(--text-strong)` (token binding live) and the `border-style:solid` preflight fix.
3. ✅ **Done** (2026-07-10). The seven primitives this spec needs, ported from `design-system/HANDOFF.md` with `_ds_bundle.js` as the reference for markup and class names: Icon, Button, Field, TextField, Select, Card, Toast. Each carries all of its required states. Satisfies **AC-6**, **AC-15**. The other 11 registry components stay `⬜ planned` (documented in `components/ui/index.ts`).
4. ✅ **Done** (2026-07-10). The API client: the typed fetch wrapper, the in memory token store, the single flight refresh with cross tab coordination, the TanStack Query client, and Zod validation at the boundary. Satisfies **AC-7**, **AC-14**.
5. ✅ **Done** (2026-07-10). The auth context, the router, the route guard, and the app shell with its header and profile menu. Satisfies **AC-7**, **AC-9**. The sign-in / register / profile route targets are placeholders until tasks 6 and 7. The guard redirect (anonymous → `/sign-in`) is wired; the round-trip back to `/profile` completes with the real sign-in form in task 6.
6. ✅ **Done** (2026-07-10). Sign in and Register: React Hook Form + Zod forms on a shared `AuthScreen` (brand, card, error banner), wired to `useAuth().signIn` / `register`. Client rules give instant inline errors (email format; password ≥ 8 on register only; role required); the submit path maps the server's `{error}` envelope to a banner through a shared `toUserMessage` helper. Sign-in returns the user to the guard's remembered `from` (default `/profile`). Satisfies **AC-8**. Verified live: both screens render on the tokens, an empty submit shows inline errors and focuses the first, and an anonymous `/profile` bounces to `/sign-in`.
7. ✅ **Done** (2026-07-10). Manage Profile (built to the UC14 UI/UX brief, since retired with the corpus): a role-aware, card-based form loaded through TanStack Query, treating 404 as the first-time empty form. Shows Academic Level to students and Instructor Type to teachers, never both. `displayName` is client-required (instant); the conditional role field stays **server-authoritative** and its `string[]` error is mapped back to the field via `mapProfileErrors`. A successful save primes the query cache, shows a quiet success toast, does not redirect, and leaves the form on the saved values. Avatar ships as a URL field with a live preview (the upload endpoint is still a follow-up). Satisfies **AC-10**, **AC-11**, **AC-12**, **AC-13**.
8. ✅ **Done** (2026-07-10). Tests (30 total, green): axe + accessibility wiring on the form primitives (TextField, Select) alongside the existing Button smoke; the `mapProfileErrors` unit test; the `RequireAuth` guard-redirect test; and a `ManageProfilePage` integration test covering role-awareness, field preservation + first-error focus, server-error mapping, and quiet-confirm-no-redirect. Satisfies **AC-6**, **AC-11**, **AC-12**, **AC-15**.
...
- `frontend/` sits outside the solution and outside CI. A green CI badge will not mean the frontend passed anything until a Node job is added. *(Resolved since: spec 0002 added the frontend CI job; the solution is now `Quiztin.sln`.)*
- `PUT /api/profile` creates the profile row on first save, so registration deliberately does not provision one. AuthService and UserService keep separate databases. *(Superseded by spec 0007: Auth and User are one Identity module over a single `identity` schema, which fixed the profile FK gap by construction — registration now creates the user row, so a first profile save no longer risks a dangling FK.)*
...
- [x] Write the YARP gateway spec. Done (spec 0002); the cookie's production `Secure=true` landed with it, and the client needed no change. Spec 0007 then folded the gateway into the single `Quiztin.Api` host, which still serves the SPA same-origin, so the posture is unchanged.
