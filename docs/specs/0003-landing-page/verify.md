...
> **Verified 2026-07-24 (first independent pass).** Driven live in the browser at `localhost:5173` plus the production build output and the test suite. **Result: pass, with one minor finding.**
...
> - **Finding (minor, AC-12 / AC-15 target size):** at 360px the primary targets meet 44px (Sign in, Get started, and every hero CTA are 44–52px), but several secondary targets measure **under** 44px on their smallest side — the hero toggle tabs (40px), the footer link list (32px), and the wordmark (26–33px). The green suite did not catch it because the axe target-size rule was not exercised. Small CSS fix (a `min-height` on the toggle tabs and footer links). This is why the spec stays **In Progress** rather than **Accepted** until the targets are bumped or the gap is accepted as known.
