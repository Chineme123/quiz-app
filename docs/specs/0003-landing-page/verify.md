...
> **Verified 2026-07-24 (first independent pass), and the one finding then fixed.** Driven live in the browser at `localhost:5173` plus the production build output and the test suite. **Result: pass.** The lone finding (a target-size gap on secondary controls) was fixed the same day and re-verified — every visible interactive target now measures ≥44px on its smallest side at both 360px and 1280px. Spec is **Accepted**.
> - **Confirmed live:** AC-1 (landing renders logged out, no redirect), AC-2 (the hero segmented control switches headline/subcopy/CTA/visual, defaults to students, no auto-rotation, exposed as an ARIA `tablist`), AC-3 (section order exact: nav → hero → how it works → for teachers → for students → AI feedback → FAQ → closing CTA → footer), AC-4 (CTAs route to `/register?role=Teacher|Student`, `/sign-in`), AC-6 (heading=Fredoka, body=Nunito, `--primary`=#4453D6 and `--accent`=#DD5530 with the "Get started" button computing to `rgb(221,85,48)`, **exactly one** gradient on the page), AC-9 (the AI-feedback example is a token-built vignette, no testimonials), AC-15 disclosure ("The people pictured on this page are illustrative and were generated for the design; they are not real Quiztin users.").
> - **Confirmed via build output:** AC-11 (the prerender carries `<title>`, full OG + Twitter + canonical + JSON-LD, and hero copy with no JS), AC-13 (`LandingPage-*.js`/`.css` split from the `index-*` entry), AC-14 (two documents; the neutral `index.html` is 984 bytes with a generic title and no OG tags; the prerender-serving `MapGet("/")` + `MapFallbackToFile("index.html")` **survived the 0007 fold into `src/Quiztin.Api/Program.cs`**).
> - **Confirmed via the passing suite (143 tests):** AC-10 motion incl. the reduced-motion path, scroll progress, and the FAQ (`motion.test.tsx`, `ScrollProgress.test.tsx`, `Faq*.test.tsx`), and the axe assertions (AC-7 contrast, AC-12).
> - **Not independently checked:** AC-5's *logged-in* nav (a session shows an app link instead of "Get started") — needs a signed-in session, out of reach for a credential-free pass.
> - **Finding (minor, AC-12 / AC-15 target size) — FIXED 2026-07-24.** At 360px the primary targets already met 44px (Sign in, Get started, and every hero CTA at 44–52px), but several secondary targets measured **under** 44px on their smallest side — the hero toggle tabs (40px), the footer link list (32px), the wordmark (26–33px), the skip link (42px), and (≥860px) the header nav links (32px). The green suite missed it because the axe target-size rule was not exercised. Fix: a 44px `min-height` (with `inline-flex`/`align-items: center` where needed) on `.qz-toggle__btn`, `.qz-footer__link`, `.qz-wordmark`, `.qz-footer__brand-mark`, `.qz-nav__link`, and `.qz-skip` in `landing.css`. Re-verified live: **0 targets under 44px** at both 360px and 1280px, no horizontal scroll, header/hero visually unchanged, and the 30 landing tests plus the full build stay green.
> - **AC-5 note:** the *logged-in* nav branch (a session shows "Go to dashboard" instead of "Get started") is implemented in `LandingNav.tsx` and confirmed by reading it; the live signed-in walk was not driven here (credential-free pass). This is the one step proven by code rather than by a running session.

The original AC-by-AC steps below are unchanged; the one stale path (AC-14 named `src/Gateway/Program.cs`) is now `src/Quiztin.Api/Program.cs` after spec 0007.
...
- **AC-10 (scroll)**: Click a nav anchor ("How it works", "Questions"): the page glides to it and the section lands below the sticky nav, not under it. A progress bar fills across the very top of the viewport as you scroll. The hero's background blobs drift against the scroll. Under `prefers-reduced-motion: reduce` the anchors jump instantly, the bar is not rendered, and the blobs sit still. The bar's tracking is asserted in `src/features/landing/motion/ScrollProgress.test.tsx`.
- **AC-10 (FAQ)**: Clicking a question glides the answer open and closed, with the chevron rotating. It is still a native `<details>`: with JavaScript disabled the browser opens and closes it, and every answer is present in the prerendered HTML. Covered by `src/features/landing/sections/Faq.test.tsx` (reduced motion, native) and `Faq.motion.test.tsx` (animated).
...
- **AC-14**: The build emits two documents: `dist/index.prerender.html` (the landing, served only at exact `/`) and `dist/index.html` (the neutral bootstrap, the SPA catch all). The host (`src/Quiztin.Api/Program.cs`, formerly `src/Gateway/Program.cs` before spec 0007) serves the prerender through an explicit `MapGet("/")` endpoint and keeps `index.html` as `MapFallbackToFile`. Run the host over the built `wwwroot` and curl: `GET /` returns the landing markup plus the SEO tags; `GET /register` and `GET /profile` return the neutral bootstrap (generic title, no landing markup or Open Graph meta). No client route other than `/` shows landing content to a no JavaScript client. `src/prerender.test.tsx` asserts the neutral bootstrap stays landing free.
