...
The prerender is done with a small postbuild script calling `react-dom/server` `renderToStaticMarkup`, which is already available through react-dom, so it adds no new package; a dedicated tool (`vite-react-ssg`) is only a fallback, and it is prototyped against the real entry first because the app builds a `createBrowserRouter` instance and renders it through `RouterProvider` with React 19 `createRoot` (and `frontend/package.json` actually pins react-router `^8.2.0`, not the v7 the context still names, a drift to reconcile before the entry is touched). The static markup is served only at exact `/`, with the neutral bootstrap `index.html` kept as the catch all SPA fallback, so the gateway's `MapFallbackToFile` does not hand the landing page's HTML to `/register` or any other deep link.
...
An independent model read the drafted spec against the actual codebase and returned a needs rework verdict. Its findings were folded in before acceptance: the gateway fallback would have served the landing markup for every route (now AC-14, with the serving fix in Feature design); `/` was already claimed by the authenticated index redirect, so mounting the landing there is a reroute (now named in build task 1); the gradient rule cited `ui-rules.md` §5 without updating it (now a required reconciliation, AC-6 and Follow-up); the prerender tool was unverified against the real router, and a zero dependency `renderToStaticMarkup` path exists (now the primary approach); and three smaller gaps (no verify step for section order, the toggle announcing only its own state, and undisclosed personas) became AC and verify additions (AC-3 check, AC-2 content announcement, AC-15).
