...
> **⚠️ AC-7 / AC-10 regressed (found 2026-07-24).** The platform was delivered and deployed live through 2026-07-20, but the spec 0007 fold was never reconciled on the Railway side, so CD has failed on every merge since and the public URL 404s. Repo-side config is correct; the fix is Railway-dashboard work. Details and the fix checklist are at the top of [verify.md](verify.md). The spec stays Accepted (correctly delivered); the deployment is the regression.
