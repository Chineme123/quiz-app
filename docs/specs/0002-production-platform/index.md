...
> **⚠️ AC-7 regressed then fixed; live URL moved (2026-07-24).** The app is **live and healthy** at **`https://quiztin-production.up.railway.app`** (the README's old `gateway-production-…` URL was the pre-0007 gateway and 404s — corrected). CD was broken 07-20 → 07-24 (Central Package Management moved versions to `Directory.Packages.props`, which the Dockerfile never copied before `dotnet restore` → `NU1015` on every deploy). **Fixed in `0c3d403`, shipped in PR #84; the deploy is green again and production is current.** Details at the top of [verify.md](verify.md). The spec stays Accepted.
