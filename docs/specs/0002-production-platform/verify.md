...
> - **Follow-up:** (1) **docs** — point the README/foundation at `quiztin-production.up.railway.app` (done 2026-07-24). (2) **CD** — get `railway up --service quiztin` green again so merges deploy; then redeploy current `main` to un-stale production. Now reachable via the local Railway CLI (authed), so this no longer strictly needs the dashboard. The spec stays **Accepted** (correctly delivered and still live); **AC-7 is regressed** (stale CD) until the deploy is fixed.
