...
> - **AC-7 (a merge to `main` deploys): FAIL — CD is broken, so the live app is stale.** The Deploy workflow has failed on the **~3 merges since 07-20** (`railway up --service quiztin --ci` fails fast in CI), so production runs 07-20 code, a few commits behind `main`. `Deploy` is not a required status check, so red deploys never blocked a merge — green CI hid the stale CD. The local Railway link also pointed at a **deleted** service id (`34edc593…`), a sign the service was recreated (now `379d0c35…`) without CI/link reconciliation. Repo-side config (`deploy.yml`, `railway.json`, the Dockerfile) is correct and the same config deployed fine on 07-20, so the break is Railway-project-side (service identity / token scope), diagnosable by re-running `railway up`.
...
> - **CD diagnosis (2026-07-24, read-only via the Railway CLI):** the break is **Railway-side, not the repo**. `railway up --service quiztin` correctly targets the service (`379d0c35`), the builder is correctly `DOCKERFILE → src/Quiztin.Api/Dockerfile`, the repo config is identical to what deployed fine on 07-20, and it is **not** a quota issue (workspace usage $1.12, no limits, not over). The failed deployment (`45ad1aa9`) is `status: FAILED, canRedeploy: true`, and its build log shows only "scheduling build on Metal builder" (twice) with **zero build output** before failing in ~9s — the signature of a transient Railway builder failure. The three failed deploys were never retried and nothing has merged to `main` since, so production stayed on the healthy 07-20 build.
> - **Follow-up:** (1) **docs** — point the README/foundation at `quiztin-production.up.railway.app` (done 2026-07-24). (2) **CD** — **redeploy** (`railway redeploy`, or a fresh `railway up --service quiztin` from `main`); the config is proven, so it will very likely succeed and un-stale production. If it fails again with the same zero-output signature, it is a Railway platform issue for their support, not a repo fix. The spec stays **Accepted** (correctly delivered and still live); **AC-7 is regressed** (stale CD) until a successful redeploy.
