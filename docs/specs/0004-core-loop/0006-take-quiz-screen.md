...
- **AC-20** *(added after `/check verify`)*: The graded record covers **every** question, not only the answered ones. Grading gives an unanswered question a blank answer that scores zero, so the results count (`N of M`) is honest for a student who skipped one, rather than reading "every one right" off only the answered rows. The results screen shows a skipped question as "Not answered", so a blank value never reads like a bug.
...
The 0005 results screen is reused, with **one small change** that `/check verify` proved necessary (this narrows the earlier "reuse it untouched" intent): grading now writes a graded row for every question, including any left unanswered (AC-20), so a skipped question is no longer missing from the record and the count cannot overstate the score. The screen renders such a row's blank answer as "Not answered". The DTO is otherwise unchanged.
