...
- **Stage:** *(updated 2026-07-24 — the line below was the original brownfield snapshot; current state follows it.)* The project began as a brownfield sprint of ~13 days (**2026-02-04 → 2026-02-17**, the original UC6/UC8/UC14 code on five microservices, two real: Quiz, User; three scaffolds). **Current state:** the full core loop is built across **specs 0001–0010** — auth + Manage Profile, classrooms (create/join), take-quiz, AI feedback + student results, quiz authoring/AI-generation/publish, and teacher classroom results — all on the **spec 0007 modular monolith** (one `Quiztin.Api` host + two modules, not five services), **deployed live on Railway**. **The core loop is walkable end to end.** Outstanding: 0009 task 7's final verify, the signed-in UI walk of the teacher results (spec 0010 is built and its backend verified live; `/check verify` covers the UI), and the student's own results index + progress (UC9), designed as [spec 0011](../docs/specs/0004-core-loop/0011-my-results/index.md) and not yet built.
...
- **React + Vite SPA** covering the loop screens. ✅ built and still widening — spec 0001 (auth, Manage Profile), 0006 (available quizzes, the take screen), 0008 (role-aware dashboards, classroom management), 0009 (quiz authoring, generation, publish), 0010 (teacher classroom results: summary, per-quiz, roll-up, drill-down).
