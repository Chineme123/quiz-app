...
> **Status:** v3.1 — **converged**. Last updated 2026-07-16. Changes from v3 (both from **spec 0006**, agreed deliberately while building the take-quiz screen, and both narrowing §69/§8 rather than reopening them): **`Abandoned` trigger 1 (time-limit expiry) no longer abandons** — an expired attempt grades the answers already saved (§69); and **Save-Draft / in-progress auto-save is pulled into v1** — it was deferred in §8, but a one-shot timed quiz that loses work on a refresh is punishing (§8). Changes from v2: three final opens resolved — `Abandoned` triggers **1–4** (pulling a one-active-attempt rule into v1); **no slice-first** (Claude-accelerated breadth, Layer-0 must-fixes still first); **database switched to PostgreSQL** (Railway-native). No open questions remain.
...
> **⚠️ Architecture superseded by [spec 0007](../docs/specs/0007-modular-monolith/index.md) (2026-07-18).** The five-microservice / YARP gateway / database-per-service design described below (§7 #10, #11, #16, #22, #30, #31; the prose in §5/§6/§9/§10) is now a **modular monolith**: one `Quiztin.Api` host serving `/api` + the SPA, two module projects (`Quiztin.Modules.Identity` = the old Auth+User merged, `Quiztin.Modules.Assessment` = the old Quiz), one `quiztin` Postgres database with a schema per module (`identity`, `quiz`), one Railway service. The Auth+User merge **fixed the profile FK gap** (§5, §10) by construction — one users table, a real FK, registration populates it (verified live: register → profile save → 200). The **Quiztin rename landed** (`Quiztin.sln`), so §7 #4 and the name note above are superseded. Rows below are kept as the historical record; where they conflict with 0007, 0007 wins.
...
- **Dev environment.** Local on the developer's **macOS** machine, now via **Docker**: `docker compose up` brings up **PostgreSQL 17** + the services (Docker Hub is reachable again as of 2026-07-13; the earlier network block is gone). The **.NET 10 SDK** + `dotnet-ef` stay installed locally (`~/.dotnet`) for host-run migrations and `dotnet test`, and the SPA dev server runs on the host (Vite) proxying to the gateway. The **GitHub Codespace / devcontainer** path was removed (2026-07-10) and is not used. Containerised local dev + the YARP gateway are settled by **spec 0002** (§7 #29–#32).
...
- **Stage:** *(updated 2026-07-24 — the line below was the original brownfield snapshot; current state follows it.)* The project began as a brownfield sprint of ~13 days (**2026-02-04 → 2026-02-17**, the original UC6/UC8/UC14 code on five microservices, two real: Quiz, User; three scaffolds). **Current state:** the full core loop is built across **specs 0001–0011** — auth + Manage Profile, classrooms (create/join), take-quiz, AI feedback + student results, quiz authoring/AI-generation/publish, teacher classroom results, and the student's own results index and progress (UC9) — all on the **spec 0007 modular monolith** (one `Quiztin.Api` host + two modules, not five services), **deployed live on Railway**. **The core loop is walkable end to end.** Outstanding: only the signed-in UI walk of the teacher results (spec 0010 is built and its backend verified live; `/check verify` covers the UI). UC9, the student's own results index and progress ([spec 0011](../docs/specs/0004-core-loop/0011-my-results/index.md)), is now **built and verified** (`/check verify`, this session).
...
- **QuizAttempt — the flagship aggregate.** Lifecycle is a real **State machine**: `NotStarted → InProgress → Submitted → Graded → Reviewable`, plus terminal `Abandoned`. Illegal transitions throw; each concrete state permits only its legal move. The state object is `[NotMapped]`, rehydrated at load from a persisted `CurrentStateName` string. **`Abandoned` is reached by triggers 2–4** *(amended by spec 0006; was 1–4)*: (1) ~~time-limit expiry~~ — **superseded: expiry no longer abandons.** Once the attempt's answers are saved server-side there is work to grade, and binning it on a timeout is off-brand for a learning tool, so an expired attempt **grades its saved answers** instead. The deadline is pinned as `ExpiresAt` at start and enforced on the draft write, so what gets graded is exactly what was saved when time ran out; (2) availability-window close — **gates starting only** *(clarified by spec 0006)*: a student already inside a quiz may finish it, so this is never evaluated on a draft save or a submit, which also stops auto-save abandoning an attempt mid-keystroke; (3) explicit student "quit" — an eager state action (domain only; no UI offers it in v1); (4) **superseded** — starting a new attempt abandons a prior unfinished one, which brings a *one-active-attempt* rule + configurable attempt limit into v1 (full multiple-attempt history stays deferred, §8). **Which abandons cost an attempt** *(spec 0006)*: only trigger 4. A superseded attempt counts against `MaxAttempts`, because restarting is the one abandon a student controls — otherwise they could restart forever and read the questions for free. A quit does not, and neither does an attempt still in progress; `AbandonReason` persists which it was. No background sweeper needed for v1 [LOCKED]. Encapsulation leaks to tighten: `TotalScore` and `RowVersion` are publicly settable.
...
| 29 | **Local development runs on `docker compose`** (Postgres 17 + services); Docker Hub is reachable again, so the container stack is the canonical local run *(spec 0002)* | One command up, and the local DB + services match the deployed image; kills the compose drift and the five hand-starts | Homebrew Postgres + hand-run services (no image parity, compose left to rot) |
...
| 35 | **Teacher classroom results are aggregated on read from the existing attempt data (no projection, no new table), owner scoped; and student names cross the module boundary through a contracts-only lookup Identity owns (`IUserDirectory`) — the first cross-module read** *(spec 0010)* | At classroom scale a few grouped reads beat a projection kept in sync on every grading event, and computing on read means no figure can go stale; the ownership check is the whole tenant boundary, since a student's academic data must stay inside their own teacher's class (`security.md` §7); and a narrow contracts assembly (an interface + a record), not a direct module reference, keeps Assessment from depending on Identity's entities (`code-standards.md` §3). The gate is ownership, not the role, so a student gets the same 404 as a missing class, never a 403 that would confirm the class exists | A results projection updated on the graded event (a second write path and a staleness risk, worth it only at cross-classroom-analytics scale); a role gate returning 403 (leaks existence, and breaks the house not-found convention); Assessment referencing the whole Identity assembly (couples the two modules' internals) |
| 36 | **The student's own results index and progress (UC9) is aggregated on read in the Assessment module, scoped to the signed-in student, reusing the spec 0010 read layer; the endpoint takes no student id (the caller is the token principal), showing the latest submitted attempt per quiz plus a per-classroom standing** *(spec 0011)* | Same forces as #35, one layer in: a student's own results are naturally small, so computing on read beats a projection (§10 defers the outbox and any projection), and taking no id makes another student's results unreachable by construction rather than by a check (`security.md` §7); the latest submitted per quiz plus a per-class standing are the useful whole without reopening the deferred full attempt history (§8); reusing 0010's read layer keeps the two results views consistent and needs no cross-module name lookup, since the data is the caller's own | A results projection on the graded event (a second write path and a staleness risk for a tiny read); listing every attempt or charting trends over time (the full multiple-attempt history deferred in §8); computing the index in the SPA from many calls (tenancy-scoped aggregation in the wrong layer) |
...
### In (v1)
- **UC2** Create Classroom + **UC3** Join Classroom — makes FR7 real. ✅ built (spec 0008).
- **UC6** Create Classroom Quiz — teacher authoring + publish, **with real AI generation** (Claude + fallback). ✅ built (manual authoring + real AI generation, spec 0009).
- **UC8** Take Classroom Quiz — attempt lifecycle + submission + idempotency, **with real scoring + real AI feedback** (Claude + fallback), **+ configurable attempt limit & one-active-attempt rule** (`Abandoned` trigger 4). ✅ built (flow, real scoring, and real AI feedback — spec 0005; the attempt-limit and one-active-attempt rule are in).
- **UC9** View My Results & Progress (student) — the student's own results index (grouped by classroom) + a per-classroom standing, aggregated on read in the Assessment module (spec 0007 folded ResultService in). The single-attempt read shipped in 0005/0006; the index + progress surface is [spec 0011](../docs/specs/0004-core-loop/0011-my-results/index.md). ✅ built and verified (`/check verify`).
- **UC10** View Classroom Results (teacher) — aggregated on read in the Assessment module (spec 0010; spec 0007 folded ResultService in). ✅ built (backend verified live; the signed-in UI walk is the one open verify).
...
- **React + Vite SPA** covering the loop screens. ✅ built and still widening — spec 0001 (auth, Manage Profile), 0006 (available quizzes, the take screen), 0008 (role-aware dashboards, classroom management), 0009 (quiz authoring, generation, publish), 0010 (teacher classroom results: summary, per-quiz, roll-up, drill-down), and 0011 (the student's own results index and progress).
...
- **Full multiple-attempt history/analytics** (v1 has only the one-active-attempt rule + attempt limit); a background `Abandoned` sweeper; domain-event outbox.
  - ~~Save-Draft; in-progress auto-save~~ — **pulled into v1 by spec 0006.** `MaxAttempts` defaults to 1, so a quiz is normally one shot: a design that loses a student's work to a refresh, or throws away answers the server is already holding when the timer runs out, is punishing in a product built to help people learn. Answers now save to the server as they are picked (one `jsonb` column on the attempt, replaced whole per save), and submit grades what is saved.
