...
- **Dev environment.** A single **GitHub Codespace (Linux)** is the canonical host, reached from multiple client machines via Remote-SSH / `gh` CLI. Devcontainer pins the toolchain (.NET 10, Docker-in-Docker; database now PostgreSQL — §7 #10).
